...
for width, lr, d in zip(widths, learning_rates, decay):
    ...
    net = nn.NeuralNetworkModel([
        nn.ConnectedLayer(input_size=4, output_size=width, activation_func='sigmoid', weight_init_method='random'),
        nn.ConnectedLayer(input_size=width, output_size=width, activation_func='sigmoid', weight_init_method='random'),
        nn.ConnectedLayer(input_size=width, output_size=1, activation_func='identity', weight_init_method='random', include_bias=False)
    ])
