...
def info_gain(data, split_attribute, target_name="Y"):
...
def predict(pruned_tree, sample):
    for attribute in pruned_tree.keys():
        value = sample[attribute]  # Removed .item()
        if value in pruned_tree[attribute]:
            pruned_tree = pruned_tree[attribute][value]
            prediction = 0
        
            if type(pruned_tree) is dict:
                prediction = predict(pruned_tree, sample)
            else:
                prediction = pruned_tree
                break
